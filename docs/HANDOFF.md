...
- 日常界面改为聊天栏、消息时间线、多行编辑器；暖色强调、低对比边框、命令面板；支持窄终端和鼠标。
- TUI 内手机号/验证码/两步验证/邮箱登录；聊天与消息搜索、历史翻页、文件夹、回复/编辑/转发/删除确认、附件发送/下载、置顶/归档/静音、话题切换。
- 按聊天与页面版本关联响应；覆盖乱序响应、发送失败保留草稿、实时消息与历史响应冲突、已删除消息复活等场景。
- 中文/Emoji 按字素编辑与显示宽度换行；草稿本地保存并同步 TDLib；断开服务后保存草稿并尝试重连，不自动重发消息。
- `tg tui --demo` 提供明确标注的虚构数据演示；`tg tui --demo --snapshot preview.svg` 导出演示终端缓冲区。
